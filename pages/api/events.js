...
async function saveEvents(events) {
  ...
  const resp = await axios.put(BASE_URL, events, { 
    headers: { 
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Accept full event object, not just symbol and date
      const symbol = (body.symbol || '').toString().trim().toUpperCase();
      const date = body.date;
      const name = body.name || symbol;
      const time = body.time || 'TBD';
      const domain = body.domain || `${symbol.toLowerCase()}.com`;
