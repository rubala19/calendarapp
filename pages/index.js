...
export default function Home() {
  ...
  async function addEvent() {
    ...
    try {
      ...
      const payload = { symbol: j.symbol, date: j.nextEarnings };
  ...
  function tileContent({ date: d, view }) {
    ...
    return (
      <div style={{ marginTop:6, display:'flex', flexDirection:'column', gap:4 }}>
        {matches.slice(0,3).map((m,i) => <div key={i} style={{ background:'#06263a', padding:'4px 6px', borderRadius:6, color:'#9fe8ff', fontWeight:700, fontSize:12 }}>{m.symbol}</div>)}
      </div>
    );
  }
  ...
  return (
    <div className="container">
      ...
      <div className="calendar-wrap">
        <div className="card">
          <Calendar value={date} onChange={setDate} tileContent={tileContent} onClickDay={onClickDay} />
        </div>
